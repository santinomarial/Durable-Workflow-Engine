    ...
    try:
        async with restarted_pool.acquire() as connection:
            await connection.execute(
                """
                update tasks set visible_at = now() - interval '1 hour'
                where workflow_id = $1 and task_type = 'timer'
                """,
                started.workflow_id,
            )
        assert await fire_due_timer(restarted_pool, queue_name="durable-timer-queue")
        assert not await fire_due_timer(restarted_pool, queue_name="durable-timer-queue")
